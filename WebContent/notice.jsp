...
String listSQL = "select postID, postTITLE, userID, date_format(postDATE,'%Y-%m-%d %H:%i') from ANNOUNCEMENT order by postID DESC;";
 ...
 <div id="warp" style="margin-top:30px">
    <div>
        ...
        <table class="table table-hover" style="text-align:center; border-top:2px solid #808080; width: 1140px;"> 
            <thead>
                <tr>
                    <th style="background-color: #FFFFFF; text-align: center; margin:0; padding: 0;">번호</th>
    ...
    <%} else {
    	do{
	    	...
	    	date.addElement(rs.getString("date_format(postDATE,'%Y-%m-%d %H:%i')"));
	    }
            	while(rs.next());
                ...
                for(int j=startrow;j<=endrow;j++) {
                    ...
                    out.println("<TR>");
                    out.println("<TD class='board_table_td' style='margin:0; padding: 0;'>"+poid.elementAt(j)+"</TD>");
                    out.println("<TD style='margin:0; padding: 0;'><a class='board_table_a' href='notice_detail.jsp?postID="+id+"'>"+title.elementAt(j)+"</a></TD>");
    ...
    %>
            ...
            <%if ((userID == null) || (userID.equals(""))) {} else if (userID.equals("admin")){ %>
				<a href = "notice_write.jsp" class="btn btn-info pull-right" style="background-color: #004c66; border: none;">글쓰기</a>
			<%} else {
			}%>
        </div>
        <div class="pagination" style="justify-content:center; text-align:center;">
        	<nav aria-label="Page navigation example">
  				<ul class="pagination">
        			<%if (wheregroup > 1) {%>
                	  <li class="page-item">                    
      					<a class="page-link" href="notice.jsp?gogroup=<%=priorgroup %>">◀</a>
    				  </li>
                	 <%} else { %>
                	  <li class="page-item">
      					<a class="page-link">◀</a>
                	 ...
                	 if (totalrows !=0) {  
                	  for(int jj=startpage; jj<=endpage; jj++) {
                	   ...
                	   <%} else { %>
                   	    <li class="page-item"><a class="page-link" href="notice.jsp?go=<%=jj%>"><%=jj %></a></li>
                	   <%}
                	  } 
                	 }
                	 if (wheregroup < totalgroup) { %>
                	  <li class="page-item">
      					<a class="page-link" href="notice.jsp?gogroup=<%=nextgroup %>">▶</a>
    				  </li>
                	 <% } else {%>
                	  <li class="page-item">
      					<a class="page-link">▶</a>
